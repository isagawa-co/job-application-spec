...
4b. **Duplicate check (MANDATORY):**
   - Read `applications/log.json` (if it does not exist yet, there are no prior applications: proceed)
...
5b. **Generate cover letter (if form has cover letter field):**
   ...
   - Draw from `profile.json` experience and question_bank fields
...
7. **Log application (MANDATORY after submission):**
   - Read `applications/log.json` (create it as an empty array `[]` if it does not exist)
...
Resume and cover letter uploads require Claude Code to be opened in this project folder. The Playwright MCP sandbox limits file access to the agent's working directory. If `resume_path` in profile.json is an absolute path outside the working directory, the upload will fail and be flagged for HITL.
